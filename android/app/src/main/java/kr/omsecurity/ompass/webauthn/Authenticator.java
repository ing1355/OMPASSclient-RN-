...
import kr.omsecurity.ompass.webauthn.exceptions.UnknownError;
import kr.omsecurity.ompass.webauthn.exceptions.*;
import kr.omsecurity.ompass.webauthn.models.*;
import kr.omsecurity.ompass.webauthn.util.*;
...
public class Authenticator {
    ...
    public AttestationObject makeCredential(AuthenticatorMakeCredentialOptions options, boolean createMode) throws WebAuthnException, VirgilException {

        // doublek
        credentialSafe.setCreateMode(createMode);
    ...
    public AttestationObject makeCredential(AuthenticatorMakeCredentialOptions options, Context ctx, CancellationSignal cancellationSignal) throws WebAuthnException, VirgilException {
        ...
        // We finish up step 3 here by checking excludeFlag at the end (so we've still gazzxotten
...
//        }
    ...
    private byte[] constructAttestedCredentialData(PublicKeyCredentialSource credentialSource) throws VirgilException {
        ...
        // doublek
        String aaguidString = "c0bf9f47-0725-4d37-8ea4-2bde998307bb";
        aaguidString = aaguidString.replace("-", "");
        byte[] aaguid = hexStringToByteArray(aaguidString);

        // doublek
        ByteBuffer credentialData = ByteBuffer.allocate(16 + 2 + credentialSource.id.length + encodedPublicKey.length);
//        ByteBuffer credentialData = ByteBuffer.allocate(16 + 2 + credentialSource.id.length + publicKeyEncoded.length);
//
        ...
        // doublek
//        credentialData.put(publicKeyEncoded);
...
//        assert toSign.length == 141 + 32;
        ...
        // doublek
        Certificate cert = this.credentialSafe.getCertificateFromKeyStore(keyPairAlias);

        // Verify for test.
        WebAuthnCryptography cryptography;
        boolean result = this.cryptoProvider.verifySignature(cert.getPublicKey(), toSign, signatureBytes);

        // construct our attestation object (attestationObject.asCBOR() can be used to generate the raw object in calling function)
        AttestationObject attestationObject = new PackedSelfAttestationObject(
                authenticatorData,
                signatureBytes,
                cert
        );
